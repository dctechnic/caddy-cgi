	...
	for _, rule := range h.Rules {
		...
		if !h.exists(fpath) || fpath[len(fpath)-1] == '/' || strings.HasSuffix(fpath, rule.Ext) {
			...
			cgih = cgi.Handler{
				...
				Dir:  h.Root,
				Root: h.Root,
				Args: []string{env["SCRIPT_FILENAME"]},
				Env:  envx,
			}
...
func Setup(c *setup.Controller) (middleware.Middleware, error) {
	...
	c.Startup = append(c.Startup, func() error {
		fmt.Println("caddy-cgi middleware is initiated")
		return nil
	})

	// Runs on Caddy shutdown, useful for cleanups.
	c.Shutdown = append(c.Shutdown, func() error {
		fmt.Println("caddy-cgi middleware is cleaning up")
		return nil
	})
...
func parse(c *setup.Controller) ([]Rule, error) {
	...
	for c.Next() {
		...
		case 3:
			...
			if err != nil {
				return rules, c.Err("Invalid cgi rule preset '" + args[2] + "'")
			}
		}
